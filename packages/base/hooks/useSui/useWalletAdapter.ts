...
export const initializeWalletAdapter = (
  standardWalletAdapter: Wallet
): IWalletAdapter => {
  const {
    features,
    icon,
  ...
  return {
    ...
    signAndExecuteTransactionBlock: async (
      ...
      try {
        ...
        throw new WalletError(
          `signAndExecuteTransactionBlock失败-${(e as any).message}`,
          ErrorCode.WALLET__SIGN_TX_ERROR
        );
      }
    },
    signMessage: async (
      ...
      try {
        ...
        throw new WalletError(
          `signMessage失败-${(e as any).message}`,
