  ...
  <div class="m-swap">
    <div class="btc-total">
      <div class="info">
        ...
        <div class="item">
          ...
          <div class="bottom">
            {{ Number(pageData?.swaped_sui || 0) / unit }} SUI
          </div>
        </div>
        <div class="item">
          <div class="top">{{ $t('home.super') }}</div>
          <div class="bottom">
            {{ Number(pageData?.swaped_shui || 0) / unit }} SHUI
          </div>
        </div>
      </div>
    ...
    <div class="eth-info">
      <div class="col col-l">
        ...
        <div class="text">
          白名单用户属于我们项目的早期支持者，可以通过 Opensea 购买
          我们发行的限量 NFT“ SHUI Whitelist ”才能加入“白名单列表”。<br />
        ...
        <div class="text">
          <ul>
            <li>
              白名单用户可以获得1SUI=100SHUI比例进行兑换，每个白名单用户兑换上限为15,000SHUI
            </li>
        ...
        <div class="text">
          为了建立良好的信用体系，我们计划将所有兑换所得SUI 全部置换成 BTC
          进行锁仓，作为项目风险控制资产由 SHUI 基金会和 SHUI DAO
          组织共同监管，同样公开透明。
        </div>
        <!-- <NDataTable bordered :columns="columns" :data="data" /> -->
        <NTable bordered>
...
import { useWallet, SuiTxBlock, formatMoney } from '@game-web/base';
import {
  CONTRACT_PACKAGE,
  SWAP_GLOBAL_ADDRESS,
  SWAP_RULE_INFO_ADDRESS
} from '@/constants';
...
const checkMoney = () => {
  coinNum.value = coinNum.value.replace(/\D|^0/g, '');
...
watch(
  ...
  (value) => {
    coinNum.value = '';
    const item = optionsInfo.value.find(
      (item) => item.id === value
    ) as TOptionsInfo;
